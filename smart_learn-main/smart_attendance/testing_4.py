...
# testing_4.py
from defaults import default_present_names, default_absent_names

now = datetime.now()
current_time = now.strftime("%I:%M:%S %p")  # 12-hour format

for name in default_present_names:
...
for name in default_absent_names:
    if name not in attendance['Name'].values:
        attendance = pd.concat([attendance, pd.DataFrame([{'Name': name, 'Time': current_time, 'Status': 'Absent'}])], ignore_index=True)
        # print(f"Face recognized as {name}.")
...
if image is None:
    ...
    for face_location in face_locations:
        ...
        if face_encodings:  # Check if a face encoding was returned
            ...
            # Check if the recognized face is not a default present name and not already marked as present
            if name not in default_present_names and name not in attendance['Name'].values:
                now = datetime.now()
    ...
    print(f"Final attendance saved to {attendance_file_name}.")
